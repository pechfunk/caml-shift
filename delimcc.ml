(**
 ...
 This is a byte-code ``native'' implementation: it does access the 
 ...
 functions in the file stacks.c. That code is heavily influenced by
 the stack-copying code of OCaml (OCaml does copy the stack to resize it)
 and by the implementation of callcc by Xavier Leroy. Again, the stack
 cannot be inspected, so we have to implement a `parallel' stack to 
 maintain the data on active prompts. 
 ...
 subcont_ek field of subcont data structure) of only 19 words. That corresponds
...
module EK :
    sig
      ...
      *)
      ...
      val dbg_print_ek_simple : ek -> unit
   ...
   struct
     ...
     *)
     ...
     external reset_ek : ek -> unit = "reset_trapsp"
     external dbg_print_ek   : ek -> unit = "dbg_print_trapsp"
     external pop_stack_fragment  : ek -> ek -> ekfragment
	 = "pop_stack_fragment"
     external push_stack_fragment_aux : ekfragment -> unit
	 = "push_stack_fragment"
     external size_stack_fragment : ekfragment -> int
	 = "size_stack_fragment"
     external ensure_stack_capacity : int -> unit
	 = "caml_ensure_stack_capacity"		 (* standard primitive *)
     ...
     let push_stack_fragment sfragment =
       ensure_stack_capacity(size_stack_fragment sfragment);
       push_stack_fragment_aux(sfragment)

     let add_ek = (+)
     let sub_ek ek1 ek2 = assert (ek1 >= ek2); ek1 - ek2

     let dbg_print_ek_simple = prerr_int
...
let take_subcont (p : 'b prompt) (f : ('a,'b) subcont -> unit -> 'b) : 'a =
  ...
  try
    ...
    raise DelimCCE			(* That will remove the prompt...*)
  with 
...
let push_delim_subcont_aux (sk : ('a,'b) subcont) (m : unit -> 'a) : 'b =
  let pb = sk.subcont_pb in
  let base = sk.subcont_bs in
  let ek = get_ek () in
  let pframe = {pfr_mark = pb.mark; pfr_ek = ek} in
  ptop := pframe :: (!ptop);
  List.iter (fun pframe ->
    ptop := {pframe with pfr_ek = add_ek ek (sub_ek pframe.pfr_ek base)} ::
         !ptop)
...
let debug_status title =
   ...
   List.iter (fun pframe -> prerr_string "\n  "; print_pfr pframe) !ptop;
   prerr_newline ();
   dbg_print_ek (get_ek ());
   prerr_newline ();
end
...
let init_global_closure () = 
  ...
  show_val 4 (Obj.repr pop_stack_fragment);
